# Osa 5: Taulukot ja debuggaus

> [!Osaamistavoitteet]
> Tämän osan jälkeen
>
> * osaat luoda taulukon, viitata sen alkioihin ja käydä sen läpi `for`- ja
>   `foreach`-silmukalla
> * osaat kirjoittaa ja testata taulukkoa käsittelevän funktion
> * osaat etsiä ohjelman virheitä debuggerilla

## Luvut

1. [Taulukot](./1-taulukot.md)
2. [Taulukot ja silmukat](./2-taulukot-ja-silmukat.md)
3. [Debuggaus](./3-debuggaus.md)

## Luennot

Luentojen numerointi on kevään 2026 toteutuksen mukainen. Uuden toteutuksen
luennot päivitetään tähän.

* [Luento 9: Debuggaus](../luennot/luento9.md)
* [Luento 10: Taulukot](../luennot/luento10.md)
* [Luento 12: Taulukkoja ja silmukoita](../luennot/luento12.md)

## Tehtävät

Tehtävät ovat lukujen sisällä sen asian kohdalla, jota ne harjoittavat. Kaikki
osan tehtävät on koottu sivulle [Osan 5 tehtävät](./tehtavat.md).

## Debuggausnäyte

Luku [Debuggaus](./3-debuggaus.md) on [debuggausnäytteen](../debuggausnayte.md)
esitietoa. Varaa näytteen suoritus tämän osan jälkeen.

## Valinnaista lisätietoa

* [Moniulotteiset taulukot](../liitteet/moniulotteiset-taulukot.md)
